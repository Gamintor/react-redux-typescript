...
const CREATE_REQUEST = 'userEvents/create-request';

interface CreateRequestAction extends Action<typeof CREATE_REQUEST> {}

const CREATE_SUCCESS = 'userEvents/create-success';
...
const CREATE_FAILURE = 'userEvents/create-failure';
...
const DELETE_REQUEST = 'userEvents/delete-request';

interface DeleteRequestAction extends Action<typeof DELETE_REQUEST> {}

const DELETE_SUCCESS = 'userEvents/delete-success';
...
const DELETE_FAILURE = 'userEvents/delete-failure';

interface DeleteFailureAction extends Action<typeof DELETE_FAILURE> {}

export const deleteUserEvent =
	(id: UserEvent['id']): ThunkAction<Promise<void>, RootState, undefined, DeleteRequestAction | DeleteSuccessAction | DeleteFailureAction> =>
	async dispatch => {
		dispatch({
			type: DELETE_REQUEST,
		});
...
const UPDATE_REQUEST = 'userEvents/update-request';

interface UpdateRequestAction extends Action<typeof UPDATE_REQUEST> {}

const UPDATE_SUCCESS = 'userEvents/update-success';

interface UpdateSuccessAction extends Action<typeof UPDATE_SUCCESS> {
	payload: { event: UserEvent };
}

const UPDATE_FAILURE = 'userEvents/update-failure';
...
export const updateUserEvent =
	...
	async dispatch => {
		...
		try {
			...
			const updatedObject: UserEvent = await response.json();
			dispatch({
				type: UPDATE_SUCCESS,
				payload: {
					event: updatedObject,
...
const userEventsReducer = (state: UserEventsState = initialState, action: LoadSuccessAction | CreateSuccessAction | DeleteSuccessAction | UpdateSuccessAction) => {
	switch (action.type) {
		...
		case DELETE_SUCCESS:
			...
			const newState = { ...state, byIds: { ...state.byIds }, allIds: state.allIds.filter(Id => Id !== id) };
